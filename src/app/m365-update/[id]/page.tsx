...
interface M365UpdatePageProps {
  params: Promise<{ id: string }>;
}

const getCachedM365Update = cache(getM365Update);

export async function generateMetadata({ params }: M365UpdatePageProps): Promise<Metadata> {
...
export default async function M365UpdatePage({ params }: M365UpdatePageProps) {
  const { id } = await params;
  const update = await getCachedM365Update(id);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex flex-col flex-grow">
            ...
            <div className="flex items-center justify-center text-[10px] text-gray-500 dark:text-gray-400 gap-1.5 py-1.5">
              ...
              {format(new Date(update.published), 'MMM d, yyyy') !==
                format(new Date(update.lastUpdated), 'MMM d, yyyy') && (
                <>
                  <span>•</span>
                  ...
                  <div className="mb-1 text-xs text-gray-700 dark:text-gray-300 flex gap-4">
                    {update.generalAvailabilityDate && (
                      <span>
                        ...
                        {format(new Date(update.generalAvailabilityDate), 'MMM d, yyyy')}
                      </span>
                    )}
                    {update.previewAvailabilityDate && (
                      <span>
                        <span className="font-semibold">Preview:</span>{' '}
                        {format(new Date(update.previewAvailabilityDate), 'MMM d, yyyy')}
                      </span>
                    )}
                  </div>
                )}
            ...
            <div className="p-6 pt-4 flex flex-col flex-grow">
              <div className="flex flex-col flex-grow">
                <div className="flex items-center gap-2 mb-1">
                  <h1 className="text-base font-medium text-gray-900 dark:text-white tracking-tight text-center">
                    {update.title}
                  </h1>
                </div>
              ...
              {update.tags && update.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {update.tags.map(tag => (
                    <span
                      key={tag}
                      className="inline-flex items-center px-2 py-1 rounded-lg text-xs font-medium bg-gray-50 text-gray-700 dark:bg-gray-700/50 dark:text-gray-300 border border-gray-200 dark:border-gray-600"
                    >
                      {tag}
                    </span>
                  ))}

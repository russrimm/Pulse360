...
function parseFeed(xml: string) {
  ...
  return items.map((item, idx) => {
    ...
    // Atom feeds use <id> instead of <guid>
    ...
    return {
      ...
      author: getText("creator") || getText("author") || getText("dc:creator") || "Visual Studio Code Team",
      categories: Array.from(item.getElementsByTagName("category")).map(cat => cat.textContent || ""),
    };
  });
}

export default function VSCodeNewsPage() {
  ...
  return (
    <ProductNewsLayout
      title="VS Code"
      description="Latest updates from the Visual Studio Code team."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item) => (
            <ProductNewsCard 
              key={item.id} 
              news={item} 
              productIcon="/icons/azure/devops/VisualStudioCode.svg"
            />
          ))}
        </div>
      )}

...
import { getTechCommunityNews } from '@/lib/api.client';
...
export default function TechCommunityNewsPage() {
  const { data: news, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['techCommunityNews'],
    queryFn: getTechCommunityNews,
  });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news?.map((item) => (
            <ProductNewsCard 
              key={item.id} 
              news={item} 
              productIcon="/icons/TechCommunity_scalable.svg"
            />
          ))}
        </div>
      )}

...
export function FabricRoadmapContent({ allPlans }: { allPlans: ReleasePlan[] }) {
  ...
  const allAreas = useMemo(() => {
    const set = new Set<string>()
    allPlans.forEach(plan => plan.investmentArea && set.add(plan.investmentArea))
    return Array.from(set).sort()
  }, [allPlans])
  ...
  const filterPlans = useCallback((plans: ReleasePlan[]) => {
    return plans.filter(plan => {
      ...
      return matchesService && matchesArea && matchesSearch && matchesDate
    })
  }, [selectedServices, selectedAreas, searchTerm, selectedDateFilter, customDateRange])
